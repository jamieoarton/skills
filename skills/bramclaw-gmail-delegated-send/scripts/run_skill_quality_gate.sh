...
refs="$(cd "$SKILL_DIR" && rg -o '`(references|assets|scripts|tests)/[A-Za-z0-9._/-]+`|\((references|assets|scripts|tests)/[A-Za-z0-9._/-]+\)' SKILL.md references/*.md tests/*.md 2>/dev/null | tr -d '`()' | cut -d: -f2 | sort -u || true)"
missing=""
if [[ -n "$refs" ]]; then
