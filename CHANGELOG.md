...
### Fixed
- Corrected `marketplace.json` format to match Anthropic structure
- Changed from GitHub source format to local path format (`source: "./"`)
- Added explicit `skills` array with all 34 skill paths
- Added `description`, `metadata`, and `strict: false` fields
